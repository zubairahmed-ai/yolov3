...
class YOLO(object):
    ...
    def saveFrame(self):
        ...
        if thistime-self.lasttime > 0.0001:
            ...
            folderPath =r"{basepath}\images\{predicted}".format(basepath=os.getcwd(), predicted=self.predictedClass)
            if not os.path.exists(folderPath):
                os.makedirs(folderPath)

            path = r"{basepath}\images\{predicted}\{timestamp}.jpg".format(basepath=os.getcwd(), predicted=self.predictedClass,timestamp=millisec)            
            cv2.imwrite(path, imageArray)
            self.lasttime = tm()
